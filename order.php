...
// FIXED: Querying structural definitions from menu_categories table to align with your exact schema configuration
                ...
                <div class="menu-items-grid">
                    <?php foreach ($cat_items as $item): ?>
                        <div class="menu-card-item menu-row-item" data-name="<?= strtolower(htmlspecialchars($item['name'])) ?>" onclick="addItemToCheckoutCart(<?= $item['id'] ?>, '<?= htmlspecialchars(addslashes($item['name'])) ?>', <?= $item['price'] ?>)">
                            <strong style="font-size:13px; display:block; color:#1e293b; min-height:34px; overflow:hidden;"><?= htmlspecialchars($item['name']) ?></strong>
...
function searchMenu() {
    ...
    document.querySelectorAll(".menu-row-item").forEach(row => {
        row.style.setProperty("display", row.getAttribute("data-name").includes(q) ? "block" : "none", "important");
    });
}
</script>

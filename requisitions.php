...
// --- CHEF INTERACTIVE SUB-FORM PIPELINE NEW PRODUCT GENERATOR ---
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action_create_chef_product"])) {
    ...
    if (!empty($item_name) && $category_id > 0) {
        // Injects entry directly with unverified flag (is_verified = 0) to protect database metrics
        $stmt = $pdo->prepare("INSERT INTO req_catalog (item_name, category_id, unit_type, unit_label, is_verified, unit_cost) VALUES (?, ?, ?, ?, 0, 0.00)");
        $stmt->execute([$item_name, $category_id, $unit_type, $unit_label]);
        $_SESSION['requisition_saved_toast'] = "Temporary product added to catalog successfully!";
    }
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action_update_requisition"])) {
    ...
    try {
        ...
        foreach ($quantities as $cat_id => $qty) {
            ...
            $new_qty = floatval($qty); // Supports decimals for weight types
...
$past_requisitions = $pdo->query("SELECT r.id, r.requested_at, r.status,
                                  (SELECT GROUP_CONCAT(CONCAT(rc.item_name, ' (x', ri.quantity, ' ', COALESCE(rc.unit_label,'Pcs'), ')') SEPARATOR ', ')
...
.sidebar-cart-row { display: flex; justify-content: space-between; align-items: center; font-size: 13px; padding: 8px 0; border-bottom: 1px solid #f7fafc; gap: 6px; }
...
.btn-block-fulfilled { background: #10b981 !important; border: 1px solid #059669 !important; }
...
.btn-block-remove { background: #ef4444 !important; border: 1px solid #dc2626 !important; }
...
.modal-input-qty { width: 60px; padding: 6px 4px; border: 1px solid #cbd5e0; text-align: center; font-size: 13px; font-weight: 700; color: #1e293b; background: #fff !important; }
                    ...
                    <table class="past-table">
                        ...
                        <tbody>
                            <?php if (!empty($past_requisitions)): foreach ($past_requisitions as $pRow):
                                ...
                                $lines = $pdo->prepare("SELECT ri.catalog_id, ri.quantity, rc.unit_type, rc.unit_label, COALESCE(ri.item_status, 'Pending') as item_status, rc.item_name as name FROM requisition_items ri JOIN req_catalog rc ON ri.catalog_id = rc.id WHERE ri.requisition_id = ?");
...
<div id="editReqModalPopup" class="modal">
    <div class="modal-content" style="background: white; max-width: 580px; width: 94%; border-radius: 12px; padding: 25px; color: #111827;">
        ...
        <h3 style="font-size: 14px; font-weight: 700; text-transform: uppercase; margin-bottom: 15px; border-bottom: 1px dashed #e2e8f0; padding-bottom: 8px;">Modify Stock Order</h3>
...
<div id="chefNewProductModal" class="modal">
    <div class="modal-content" style="background: white; max-width: 440px; width: 90%; border-radius: 12px; padding: 25px; color: #111827;">
        ...
        <form method="POST" action="requisitions.php" style="margin: 0;">
            ...
            <div style="margin-bottom: 12px;">
                <label style="font-size: 11px; font-weight: 700; color: #4b5563; display: block; margin-bottom: 4px;">Product Name</label>
...
window.openEditRequisitionModal = function(reqId, element) {
    ...
    try {
        ...
        container.innerHTML = items.map(i => {
            ...
            const unitLabel = i.unit_label || 'Pcs';
            ...
            // REQ 2: Metric options generated dynamically depending on the item configuration type
            let unitDropdownMarkup = '';
            if (unitType === 'Weight') {
                unitDropdownMarkup = `
                    <select name="req_item_unit_label[${i.catalog_id}]" class="unit-selector-dropdown">
                        <option value="kg" ${unitLabel === 'kg' ? 'selected' : ''}>kg</option>
                        <option value="gms" ${unitLabel === 'gms' ? 'selected' : ''}>gms</option>
                        <option value="Ltr" ${unitLabel === 'Ltr' ? 'selected' : ''}>Ltr</option>
                        <option value="ml" ${unitLabel === 'ml' ? 'selected' : ''}>ml</option>
                    </select>
                `;
            } else {
                unitDropdownMarkup = `
                    <select name="req_item_unit_label[${i.catalog_id}]" class="unit-selector-dropdown">
                        <option value="Pcs" ${unitLabel === 'Pcs' ? 'selected' : ''}>Pcs</option>
                        <option value="Packets" ${unitLabel === 'Packets' ? 'selected' : ''}>Packets</option>
                        <option value="Boxes" ${unitLabel === 'Boxes' ? 'selected' : ''}>Boxes</option>
                    </select>
                `;
            }

            // Determines the incremental size factor for custom metric counters
            const calculationStepFactor = (unitType === 'Weight') ? 0.25 : 1;
                ...
                <div id="itemVerificationRow_${i.catalog_id}" class="verification-item-row-wrapper ${rowStateClass}" style="display:flex; justify-content:space-between; align-items:center; padding:12px 10px; border-bottom:1px solid #e2e8f0; background:#ffffff; margin-bottom:6px; border-radius:8px; gap:8px;">
                    ...
                    <div style="display:flex; align-items:center; gap:6px;">
                        <div class="modal-qty-container">
                            <button type="button" class="modal-qty-btn" onclick="window.adjustVerificationRowQty(${i.catalog_id}, -${calculationStepFactor})">-</button>
                            <input type="number" id="mdlQtyInput_${i.catalog_id}" name="req_item_qty[${i.catalog_id}]" value="${i.quantity}" min="0" step="${calculationStepFactor}" class="modal-input-qty" oninput="window.handleQuantityInputChangeDirect(${i.catalog_id})">
                            <button type="button" class="modal-qty-btn" onclick="window.adjustVerificationRowQty(${i.catalog_id}, ${calculationStepFactor})">+</button>
                        </div>
                        ${unitDropdownMarkup}
                    </div>
